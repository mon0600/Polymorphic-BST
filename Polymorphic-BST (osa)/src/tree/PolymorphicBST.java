...
public class PolymorphicBST<K extends Comparable<K>, V>  {
	...
	public Set<K> keySet() {
		//	Set<?> rnmd = new HashSet();
		//	for ()
		//	
		Set<K> rndm = new HashSet<K>();
		root.addKeysToCollection(rndm);
		return rndm;
	}
	...
	public PolymorphicBST<K, V> subMap(K fromKey, K toKey) {
		PolymorphicBST<K,V> ptree = new PolymorphicBST<K,V>();
		ptree.root = this.root.subTree(fromKey, toKey);
		return ptree;
	}
